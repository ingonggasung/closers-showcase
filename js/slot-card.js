/* ── slot-card.js ─────────────────────────────────────────────────────────────
   게시글 카드 하나를 만드는 곳. 목록이 보이는 모든 페이지가 이걸 씁니다.

   - createSlotCard()     : 카드 하나(이미지 캐러셀 + 제목 + 작성자 + 배지).
   - renderSlotMasonry()  : 카드들을 세로 열에 나눠 담는 벽돌식 배치.
                            최신 글이 왼쪽 위, 오래될수록 오른쪽 아래로 갑니다.
   - getMasonryColumns()  : 화면 폭에 따라 열 수 결정 (모바일 3, PC 5).
   - slotMatchesQuery()   : 검색. 제목·게시자·코스튬명 중 선택한 기준으로 비교.

   캐러셀 높이 처리가 이 파일에서 제일 까다로운 부분입니다. 카드 가로폭은
   고정이고 세로는 이미지 원본 비율을 따라가야 해서, 보이는 이미지의 실제
   높이를 재서 감싸는 요소에 직접 지정합니다. CSS 로는 안 되는 이유는 아래
   syncCarouselHeight() 주석에 적어뒀습니다.
   ────────────────────────────────────────────────────────────────────────── */
